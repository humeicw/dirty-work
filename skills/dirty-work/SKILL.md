---
name: dirty-work
description: Read this before the first file read, grep or shell command of a task. Decide what goes to a subagent instead of you, which model tier it needs, and how to write the work order. Use it when the user says "why did this break", "the build broke", "dig into it", "figure out what happened", "where is X used", "does anything else use", "find all the places", "have a look at the repo", "is this in good shape", "go through these logs", "rename it everywhere", "refactor", "migrate", "add this feature" - anything that means investigating, searching code, reading a pile of files or logs, a repo-wide rewrite, or building across several files. It applies even when the request sounds tiny - "quick question" and "just check" are the ones that turn into reading the whole repo. Also use on "delegate this", "use subagents", "save context", "my context is filling up", or which model to use. Not for one named edit, one named function or file, a general question, or a document you are co-writing with the user turn by turn.
---
