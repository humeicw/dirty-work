...
这个技能把调查和大批量阅读派给子 Agent（帮主 Agent 干活的帮手），只收回结论、不收回原始材料，并把每件活派给刚好能干好它的最小一档模型。名字取自英文固定说法 *do the dirty work*：脏活派出去，责任不派出去。
...
同一句话："查一下上周合并之后构建为什么挂了。"

| 没装这个技能 | 装了之后 |
|---|---|
| 先全仓库搜一遍，读 CI 配置，打开六个源文件，把构建日志最后 400 行拉出来，全部堆进主对话。 | 先判断：这是调查，必须派出去。一个子 Agent 拿到问题和该翻的地方。 |
| 一小时后，它中途碰过的每个文件还留在上下文里，和你真正的问题抢注意力。 | 带回来的是根本原因、真正相关的那几行，以及它没能确定的部分。日志留在子 Agent 自己那边。 |
| 整件事跑在你开会话时选的那个模型上。 | 改个名交给最便宜的一档；推根本原因就不能这么省。 |
| 等它说"上下文快满了，正在压缩"的时候你才发现。 | 主对话还留着地方，让你做本来想做的那个决定。 |

这一节讲的是机制，不是实测数据。见[我实测了什么](#我实测了什么)。
...
技能正文里的同一张表还有一列"模型"，要你按自己手头的模型填一次。档位定义在不同工具之间通用，模型名字过几个月就过时。4 档把方向定下来之后，剩下的实现降到 3 档，范围明确的改动降到 2 档，机械性的活降到 1 档。
...
- 子 Agent 一共处理了约 1900 万 token 的材料，回到主对话的只有约 4.09 万 token，占 0.21%；每次派活带回来的内容，中位数是 272 token。
- 88% 的文件读取和命令执行发生在子 Agent 那边。
- 档位分布：3 档占 78%，1 档和 2 档合计 20%，4 档 2%。我的活大多是多步相扣的，所以便宜档在这里是少数。

这是真实工作记录里的观察数据，任务各不相同，不是对照实验：它说明有多少东西没进主对话，不说明结果好了多少。token 数是估算值，每份材料只算一次，不把缓存重读重复计入。

## 验证范围与局限

这套决策规则有日常使用撑着，我每天在 Claude Code 和 Codex 上用它的私有版本。**但装它的这个盒子只做过一次加载测试**：用 `claude --plugin-dir` 加载，技能和七个子 Agent 都能注册，1 档和 4 档各成功调用了一次。还没有人跑过 `/plugin marketplace add` 或 `npx skills add`。其他支持 Agent Skills 的工具应该能用，因为技能是纯文字，但我没试过。
...
- **子 Agent 人设加一套编排运行时**：它们回答的是另一个问题——帮手应该长什么样。
...
**这名字听着像是把讨厌的活甩给别人。**
这就是那个玩笑，而且只是半个玩笑。读文件、搜代码这些活确实甩出去了，甩不出去的是责任：工单是主 Agent 写的，答案够不够格是主 Agent 判的，最后向你交代的还是主 Agent。

**派子 Agent 不是更费 token 吗？**
总量上通常确实更费，因为每个子 Agent 都要重新读一遍背景。少掉的是主对话里的 token，而主对话是那个必须连续几个小时保持清醒的地方。如果你的目标是把账单总额降下来，这个技能不合适。
...
如果它帮你少压缩了一次上下文，点个 star 能让下一个人更容易找到它。如果没帮上，请去开一个 issue——那比一个 star 对我有价值得多。
